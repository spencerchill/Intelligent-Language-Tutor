...
class Application:
    ...
    def app(self):
        ...
        def highlight_text(incorrect_indices):
            """Highlight incorrect letters in the text"""
            text_display.config(state="normal")
            text_display.tag_remove("highlight", "1.0", tk.END)  # previous highlights

            for index in incorrect_indices:
                text_display.tag_add("highlight", f"1.{index}", f"1.{index+1}")
        ...
        def process_audio(filename):
            ...
            error_info = ed.get_pronunciation_score(current_text, tp.text_to_phoneme(current_text), user_phonemes)
            print("Pronunciation info:", error_info)
           
            highlight_text(error_info['incorrect_indices'])
        ...
        current_phoneme = tp.text_to_phoneme(current_text)
        ...
        text_display.config(state="disabled")  
        text_display.tag_configure("highlight", foreground="red")
        # feel free to remove text_label, right now it prints both on screen
        text_label = tk.Label(root, text=current_text, font=("Arial", 14))
        ...
        def generate():
            ...
            current_text = ''.join([char for char in gen_random_text() if char not in string.punctuation])
            current_phoneme = tp.text_to_phoneme(current_text)
            text_label.config(text=current_text) # remove if happy with text display :)
